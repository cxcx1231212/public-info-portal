...
async function pushRecommendedSitesToTargets(env){
  ...
  const targets=await env.DB.prepare("SELECT * FROM ad_sync_targets WHERE enabled=1 AND name='跑狗论坛' ORDER BY id").all(),payload=await makeRecommendedSitesSyncPayload(env);
