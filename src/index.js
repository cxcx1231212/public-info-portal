...
async function makeRecommendedSitesSyncPayload(env){await ensureRecommendedSites(env);const sites=await env.DB.prepare('SELECT id,name,site_url,sort_order,enabled,updated_at FROM recommended_sites ORDER BY sort_order,id').all();return {type:'recommended-sites',version:Date.now(),generated_at:new Date().toISOString(),recommended_sites:sites.results||[]};}
async function pushRecommendedSitesToTargets(env){
  await ensureAdSyncSchema(env);
  const targets=await env.DB.prepare("SELECT * FROM ad_sync_targets WHERE enabled=1 AND receiver_url LIKE '%lhtxpgltnew.%' ORDER BY id").all(),payload=await makeRecommendedSitesSyncPayload(env);
  return Promise.all((targets.results||[]).map(async target=>{try{const receiver=new URL(cleanText(target.receiver_url,1000));receiver.pathname='/recommended-sites-sync.php';const response=await fetch(receiver.toString(),{method:'POST',headers:{'content-type':'application/json','x-ad-sync-token':target.sync_token},body:JSON.stringify(payload)}),result=await response.json().catch(()=>({})),ok=response.ok&&result.success!==false,message=cleanText(result.message||(ok?'连准网站已推送':'接收站返回失败'),500);return {id:target.id,name:target.name,success:ok,message};}catch(error){return {id:target.id,name:target.name,success:false,message:cleanText(error?.message||'无法连接接收站',500)};}}));
}
...
async function ensureLotteryContent(env,lotteryType){
  ...
  await ensureSpecialMaterialCoverage(env,lotteryType);
}
async function homepageInitialData(env){
  await Promise.all([1,5,8].map(type=>ensureLotteryContent(env,type)));
  const byType={1:[],5:[],8:[]};
  try{
  ...
  const specialMaterialsByType={1:[],5:[],8:[]};
  try{for(const type of [1,5,8])await ensureSpecialMaterialCoverage(env,type);const special=await env.DB.prepare('SELECT * FROM special_materials WHERE lottery_type IN (1,5,8) AND enabled=1 ORDER BY lottery_type,board_key,period DESC,id DESC LIMIT 200').all();for(const row of special.results||[])(specialMaterialsByType[validLotteryType(row.lottery_type)]||specialMaterialsByType[5]).push(row);}catch(error){console.error('homepage_special_materials_failed',error?.message||error);}
  const masterPostsByType={1:[],5:[],8:[]},memberPostsByType={1:[],5:[],8:[]};
  try{await Promise.all([ensureMasterCatalog(env),ensureMemberPostsSchema(env)]);const [posts,members]=await Promise.all([env.DB.prepare('SELECT p.*,m.name,m.avatar,m.rank_no,m.specialty FROM master_posts p JOIN masters m ON m.id=p.master_id WHERE m.enabled=1 AND p.lottery_type IN (1,5,8) ORDER BY p.lottery_type,p.period DESC,m.rank_no LIMIT 2400').all(),env.DB.prepare('SELECT * FROM member_posts WHERE enabled=1 AND lottery_type IN (1,5,8) ORDER BY id DESC LIMIT 300').all()]);for(const row of posts.results||[])(masterPostsByType[validLotteryType(row.lottery_type)]||masterPostsByType[5]).push(row);for(const row of members.results||[])(memberPostsByType[validLotteryType(row.lottery_type)]||memberPostsByType[5]).push(row);}catch(error){console.error('homepage_master_posts_failed',error?.message||error);}
  return {contentByType:byType,specialMaterialsByType,masterPostsByType,memberPostsByType};
}
...
const SPECIAL_MATERIAL_BOARDS={two_code:9,inside_code:5,sixgroup:3};
const specialMaterialSeed={two_code:[[250,{picks:[['蛇',14],['虎',17]]}],[251,{picks:[['牛',30],['马',1]]}],[252,{picks:[['狗',9],['鸡',22]]}],[253,{picks:[['兔',16],['猴',23]]}],[254,{picks:[['蛇',2],['羊',12]]}],[255,{picks:[['猪',44],['牛',18]]}],[256,{picks:[['马',1],['龙',3]]}],[257,{picks:[['鼠',7],['龙',15]]}],[258,{picks:[]}]],inside_code:[[254,{numbers:[2,43]}],[255,{numbers:[22,44]}],[256,{numbers:[1,33]}],[257,{numbers:[27,7]}],[258,{numbers:[]}]],sixgroup:[[256,{groups:[[3,26,25],[9,37,7],[26,44,35],[4,46,21],[14,26,47],[40,15,37]],open:[40,15,9,37,7,3]}],[257,{groups:[[47,39,27],[11,36,47],[8,46,27],[33,25,26],[14,24,47],[8,45,39]],open:[8,46,47,39,27,45]}],[258,{groups:[],open:[]}]]};
async function ensureSpecialMaterialsSchema(env){await env.DB.prepare("CREATE TABLE IF NOT EXISTS special_materials(id INTEGER PRIMARY KEY AUTOINCREMENT,lottery_type INTEGER NOT NULL,board_key TEXT NOT NULL,period INTEGER NOT NULL,content_json TEXT NOT NULL DEFAULT '{}',status TEXT NOT NULL DEFAULT 'pending',enabled INTEGER NOT NULL DEFAULT 1,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE(lottery_type,board_key,period))").run();await env.DB.prepare('CREATE INDEX IF NOT EXISTS idx_special_materials_lookup ON special_materials(lottery_type,board_key,enabled,period DESC,id DESC)').run();const seeded=await env.DB.prepare("SELECT setting_value FROM site_settings WHERE setting_key='special_materials_seed_v1' LIMIT 1").first();if(!seeded){const rows=[];for(const [board,records]of Object.entries(specialMaterialSeed))for(const[period,data]of records)rows.push(env.DB.prepare("INSERT OR IGNORE INTO special_materials(lottery_type,board_key,period,content_json,status,enabled) VALUES(?,?,?,?,?,1)").bind(5,board,period,JSON.stringify(data),period===258?'pending':'win'));if(rows.length)await env.DB.batch(rows);await env.DB.prepare("INSERT INTO site_settings(setting_key,setting_value,updated_at) VALUES('special_materials_seed_v1','1',CURRENT_TIMESTAMP) ON CONFLICT(setting_key) DO UPDATE SET setting_value='1',updated_at=CURRENT_TIMESTAMP").run();}}
function specialMaterialPayload(board,period,seed,lotteryType){const random=rngFor(period*7001+(Number(seed)||0)+lotteryType*300001),number=()=>sample(numberPool,1,random)[0];if(board==='two_code')return{picks:sample(zodiacOrder,2,random).map(zodiac=>[zodiac,number()])};if(board==='inside_code')return{numbers:[number(),number()]};return{groups:Array.from({length:6},()=>sample(numberPool,3,random)),open:[]};}
async function ensureSpecialMaterialPeriod(env,period,seed,lotteryType){if(!period)return;lotteryType=validLotteryType(lotteryType);await ensureSpecialMaterialsSchema(env);const inserts=Object.keys(SPECIAL_MATERIAL_BOARDS).map(board=>env.DB.prepare("INSERT OR IGNORE INTO special_materials(lottery_type,board_key,period,content_json,status,enabled) VALUES(?,?,?,?, 'pending',1)").bind(lotteryType,board,period,JSON.stringify(specialMaterialPayload(board,period,seed,lotteryType))));if(inserts.length)await env.DB.batch(inserts);const cleanup=Object.entries(SPECIAL_MATERIAL_BOARDS).map(([board,count])=>env.DB.prepare('DELETE FROM special_materials WHERE lottery_type=? AND board_key=? AND id NOT IN (SELECT id FROM special_materials WHERE lottery_type=? AND board_key=? AND enabled=1 ORDER BY id DESC LIMIT ?)').bind(lotteryType,board,lotteryType,board,count));if(cleanup.length)await env.DB.batch(cleanup);}
async function historyDraws(env,lotteryType){
  const year=new Date().getFullYear(),response=await fetch('https://6htv70.com/gallerynew/h5/lottery/search?pageNum=1&pageSize=50&year='+year+'&sort=1&lotteryType='+lotteryType,{headers:{accept:'application/json','user-agent':'Mozilla/5.0'}});if(!response.ok)throw new Error('开奖记录 HTTP '+response.status);const payload=await response.json(),found=[],seen=new Set();
...
function specialHistoryRows(board,draws,lotteryType){
  const historyCount={two_code:8,inside_code:4,sixgroup:2}[board]||0,selected=draws.slice(-historyCount);return selected.map(draw=>{const special=draw.balls[6],position=draws.findIndex(item=>item.period===draw.period);let prior=special;for(let index=position-1;index>=0;index--){const candidate=draws[index]?.balls?.[6];if(candidate&&Number(candidate.number)!==Number(special.number)){prior=candidate;break;}}const regular=draw.balls.slice(0,6).map(ball=>ball.number),random=rngFor(draw.period*7919+lotteryType*97),outside=numberPool.filter(number=>!regular.includes(number)),randomGroup=()=>[regular[Math.floor(random()*regular.length)],...sample(outside,2,random)];let data={};if(board==='two_code')data={picks:[[special.zodiac,special.number],[prior.zodiac,prior.number]]};else if(board==='inside_code')data={numbers:[special.number,prior.number]};else data={groups:[regular.slice(0,3),regular.slice(3,6),randomGroup(),randomGroup(),randomGroup(),randomGroup()],open:regular};return {period:draw.period,data};});
}
async function syncSpecialMaterialsFromHistory(env,lotteryType){
  ...
  for(const board of Object.keys(SPECIAL_MATERIAL_BOARDS)){statements.push(env.DB.prepare('DELETE FROM special_materials WHERE lottery_type=? AND board_key=?').bind(lotteryType,board));for(const item of specialHistoryRows(board,draws,lotteryType))statements.push(env.DB.prepare("INSERT INTO special_materials(lottery_type,board_key,period,content_json,status,enabled) VALUES(?,?,?,?, 'win',1)").bind(lotteryType,board,item.period,JSON.stringify(item.data)));statements.push(env.DB.prepare("INSERT INTO special_materials(lottery_type,board_key,period,content_json,status,enabled) VALUES(?,?,?,?, 'pending',1)").bind(lotteryType,board,nextPeriod,JSON.stringify(board==='two_code'?{picks:[]}:board==='inside_code'?{numbers:[]}:{groups:[],open:[]})));}
...
async function ensureSpecialMaterialCoverage(env,lotteryType){
  ...
  if(force)await env.DB.prepare("INSERT INTO site_settings(setting_key,setting_value,updated_at) VALUES('special_materials_history_version','draw-history-v4',CURRENT_TIMESTAMP) ON CONFLICT(setting_key) DO UPDATE SET setting_value='draw-history-v4',updated_at=CURRENT_TIMESTAMP").run();
}
...
async function publicApi(request,env,url){
  ...
  if(url.pathname==='/api/public/special-materials'&&request.method==='GET'){const lotteryType=validLotteryType(url.searchParams.get('lotteryType'));await ensureLotteryContent(env,lotteryType);await ensureSpecialMaterialCoverage(env,lotteryType);const result=await env.DB.prepare("SELECT * FROM special_materials WHERE lottery_type=? AND enabled=1 ORDER BY board_key,period DESC,id DESC").bind(lotteryType).all();return json({success:true,data:result.results||[]});}
  if(url.pathname==='/api/public/site-settings'&&request.method==='GET'){const result=await env.DB.prepare("SELECT setting_key,setting_value FROM site_settings WHERE setting_key='registration_url'").all(),settings={};for(const row of result.results||[])settings[row.setting_key]=row.setting_value;return json({success:true,data:settings},200,{'cache-control':'no-store','access-control-allow-origin':'*'});}
  if(url.pathname==='/api/public/member-post-settings'&&request.method==='GET'){const result=await env.DB.prepare("SELECT setting_value FROM site_settings WHERE setting_key='member_post_current_url'").first();return json({success:true,member_post_current_url:result?.setting_value||''},200,{'cache-control':'no-store'});}
...
async function adminApi(request,env,url){
  ...
  if(request.method==='POST'){
    ...
    const result=await env.DB.prepare('INSERT INTO '+resource.table+' ('+fields.join(',')+') VALUES ('+fields.map(()=>'?').join(',')+')').bind(...fields.map(field=>input[field])).run(),syncResults=match[1]==='links'?await pushRecommendedSitesToTargets(env):[];return json({success:true,id:result.meta.last_row_id,sync:syncResults});
  }
  if(request.method==='PUT'&&id){
    ...
    await env.DB.prepare('UPDATE '+resource.table+' SET '+fields.map(field=>field+'=?').join(',')+',updated_at=CURRENT_TIMESTAMP WHERE id=?').bind(...fields.map(field=>input[field]),id).run();const syncResults=match[1]==='ads'?await pushAdsToTargets(env,url.origin):match[1]==='links'?await pushRecommendedSitesToTargets(env):[];return json({success:true,sync:syncResults});
  }
  if(request.method==='DELETE'&&id){await env.DB.prepare('DELETE FROM '+resource.table+' WHERE id=?').bind(id).run();const syncResults=match[1]==='ads'?await pushAdsToTargets(env,url.origin):match[1]==='links'?await pushRecommendedSitesToTargets(env):[];return json({success:true,sync:syncResults});}
  return json({success:false,message:'请求方式不支持'},405);
}
