...
async function generateNextPeriod(env,period,seed,lotteryType=5){
  ...
  lotteryType=validLotteryType(lotteryType);await ensureContentLotteryType(env);await ensureSpecialMaterialPeriod(env,period,seed,lotteryType);
...
async function ensureSpecialMaterialCoverage(env,lotteryType){lotteryType=validLotteryType(lotteryType);await ensureSpecialMaterialsSchema(env);const max=await env.DB.prepare("SELECT MAX(period) period FROM content_items WHERE lottery_type=? AND enabled=1 AND section_key<>'threeperiod'").bind(lotteryType).first(),period=Number(max?.period||0);if(!period)return;for(const board of Object.keys(SPECIAL_MATERIAL_BOARDS)){const existing=await env.DB.prepare('SELECT COUNT(*) count FROM special_materials WHERE lottery_type=? AND board_key=? AND enabled=1').bind(lotteryType,board).first();if(Number(existing?.count||0))continue;for(let i=SPECIAL_MATERIAL_BOARDS[board]-1;i>=0;i--)await ensureSpecialMaterialPeriod(env,period-i,period-i,lotteryType);}}
