(function () {
  ...
  function syncLotterySectionTitles(lotteryType){
    const lotteryNames={1:'香港六合彩',5:'澳门六合彩',8:'天天六合彩'},name=lotteryNames[lotteryType]||'澳门六合彩';
    document.querySelectorAll('[data-section-key]:not([data-section-key="five"]) h2').forEach(title=>{
      const base=title.dataset.baseTitle||title.textContent.trim();title.dataset.baseTitle=base;
      if(base.includes('123六合网'))title.textContent=base.replace('123六合网',name);
    });
  }
  ...
  function boot() {
    ...
    if (file === 'index.html' || file === '') {
      ...
      syncLotterySectionTitles(currentType());
      ...
      document.querySelectorAll('#lotteryMenu button, .lottery-tab').forEach(button => button.addEventListener('click', () => {
        ...
        syncLotterySectionTitles(selected);
