...
(function(){
  ...
  const style=document.createElement('style');style.id='promotionEffects';style.textContent='@keyframes promoEdgeMove{0%,100%{transform:translateY(0) rotate(-4deg)}45%{transform:translateY(-13px) rotate(4deg)}60%{transform:translateY(0) rotate(-2deg)}}@keyframes promoRainFall{0%{transform:translate3d(0,-70px,0) rotate(-10deg);opacity:0}10%{opacity:1}100%{transform:translate3d(var(--drift),110vh,0) rotate(380deg);opacity:.94}}.promo-edge-redpacket{position:fixed;z-index:9997;right:12px;bottom:75px;display:grid;width:54px;height:72px;place-items:center;border:2px solid #ffd766;border-radius:10px;background:linear-gradient(145deg,#ff6060,#d70b22 65%,#a00013);box-shadow:0 5px 13px rgba(160,0,19,.45);color:#ffe993;font:900 13px/1.15 Arial,sans-serif;text-align:center;text-decoration:none;animation:promoEdgeMove .85s ease-in-out infinite}.promo-edge-redpacket b{display:block;font-size:20px}.promo-rain-layer{position:fixed;z-index:9996;inset:0;pointer-events:none;overflow:hidden}.promo-rain-packet{position:absolute;top:0;display:grid;width:31px;height:42px;place-items:center;border:1px solid #ffe477;border-radius:6px;background:linear-gradient(145deg,#ff5156,#c8051e);box-shadow:0 3px 8px rgba(177,0,20,.4);color:#ffe470;font:900 17px/1 Arial,sans-serif;text-decoration:none;pointer-events:auto;animation:promoRainFall var(--fall) linear forwards}@media(max-width:520px){.promo-edge-redpacket{right:8px;bottom:65px;width:48px;height:64px;font-size:12px}.promo-edge-redpacket b{font-size:18px}}@media(prefers-reduced-motion:reduce){.promo-edge-redpacket,.promo-rain-packet{animation:none}}';document.head.appendChild(style);
  ...
  document.body.append(edge,rain);
