...
(function(){
  ...
  function renderFormula(){
    ...
    fetch('/fdata.html?lotteryType='+type(),{cache:'no-store'}).then(r=>r.ok?r.json():Promise.reject()).then(payload=>{
      ...
      list.innerHTML=rows.map(row=>{const label=row.cardName||'规律推荐',path=String(row.href||''),href=path.startsWith('/posts/')?'https://txgs888.q3665.com'+path:'#',image=row.thumbnailUrl||row.imageUrl||'/formula-placeholder.svg';return '<a class="formula-card" href="'+esc(href)+'" target="_blank" rel="noopener"><span class="formula-cover"><img src="'+esc(image)+'" alt="'+esc(label)+'"></span><strong class="formula-name">'+esc(label)+'</strong></a>';}).join('');
