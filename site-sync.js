...
(function(){
  ...
  const style=document.createElement('style');style.id='promotionEffects';style.textContent='@keyframes promoEdgeMove{0%{transform:translateX(-8px) rotate(-4deg)}100%{transform:translateX(28px) rotate(4deg)}}@keyframes promoJump{0%,100%{transform:translateY(0) scale(1)}45%{transform:translateY(-13px) scale(1.06)}60%{transform:translateY(0) scale(.96)}}@keyframes promoRainFall{0%{transform:translate3d(0,-70px,0) rotate(-10deg);opacity:0}10%{opacity:1}100%{transform:translate3d(var(--drift),110vh,0) rotate(380deg);opacity:.94}}.promo-edge-redpacket{position:fixed;z-index:9997;left:0;top:37%;display:grid;width:54px;height:72px;place-items:center;border:2px solid #ffd766;border-radius:10px;background:linear-gradient(145deg,#ff6060,#d70b22 65%,#a00013);box-shadow:0 5px 13px rgba(160,0,19,.45);color:#ffe993;font:900 13px/1.15 Arial,sans-serif;text-align:center;text-decoration:none;animation:promoEdgeMove 1.4s ease-in-out infinite alternate}.promo-edge-redpacket b{display:block;font-size:20px}.promo-win-report{position:fixed;z-index:9997;right:12px;bottom:75px;display:flex;align-items:center;gap:7px;padding:9px 12px;border:2px solid #ffe16b;border-radius:24px;background:linear-gradient(135deg,#ff3e43,#ca061e);box-shadow:0 5px 14px rgba(183,0,20,.42);color:#fff;font:900 14px/1.1 Arial,sans-serif;text-decoration:none;animation:promoJump .85s ease-in-out infinite}.promo-win-report span{display:grid;width:29px;height:29px;place-items:center;border-radius:50%;background:#fff2a2;color:#e10d24;font-size:18px}.promo-rain-layer{position:fixed;z-index:9996;inset:0;pointer-events:none;overflow:hidden}.promo-rain-packet{position:absolute;top:0;display:grid;width:31px;height:42px;place-items:center;border:1px solid #ffe477;border-radius:6px;background:linear-gradient(145deg,#ff5156,#c8051e);box-shadow:0 3px 8px rgba(177,0,20,.4);color:#ffe470;font:900 17px/1 Arial,sans-serif;text-decoration:none;pointer-events:auto;animation:promoRainFall var(--fall) linear forwards}@media(max-width:520px){.promo-edge-redpacket{top:31%;width:48px;height:64px;font-size:12px}.promo-edge-redpacket b{font-size:18px}.promo-win-report{right:8px;bottom:65px;padding:8px 10px;font-size:13px}}@media(prefers-reduced-motion:reduce){.promo-edge-redpacket,.promo-win-report,.promo-rain-packet{animation:none}}';document.head.appendChild(style);
  const edge=document.createElement('a');edge.className='promo-edge-redpacket';edge.href='#';edge.innerHTML='限时<b>红包</b>点击领取';
  const report=document.createElement('a');report.className='promo-win-report';report.href='#';report.innerHTML='<span>喜</span>中奖喜报';
  const rain=document.createElement('div');rain.className='promo-rain-layer';
  const go=event=>{event.preventDefault();window.open(target,'_blank','noopener,noreferrer');};edge.addEventListener('click',go);report.addEventListener('click',go);
  document.body.append(edge,report,rain);
...
(function(){
  ...
  function renderFormula(){
    ...
    const renderDirectCards=()=>{const cards=[['一肖中特','zodiac','1'],['三中特网','zodiac','3'],['六肖中特','zodiac','6'],['九肖中特','zodiac','9'],['三中三','tema','3'],['八码中特','tema','8'],['十码中特','tema','10'],['十八码中特','tema','18'],['平特一肖','pingte','one'],['二中二','fushi','22'],['三中三连','fushi','33'],['二连肖','fushi','2x'],['三连肖','fushi','3x'],['特码单双','danshuang',''],['特码波色','wave',''],['特码五行','wuxing',''],['家野中特','jiaye',''],['杀码','kill','code'],['杀肖','kill','animal'],['杀尾','kill','tail'],['杀头','kill','head'],['杀波','kill','wave'],['特码大小','size',''],['尾数','tail',''],['头数','head','']];list.innerHTML=cards.map(([label,board,category])=>{const image='https://txgs888.q3665.com/api/formula-recommendations/thumbnail?lotteryType='+type()+'&board='+board+'&category='+category+'&cardName='+encodeURIComponent(label);return '<a class="formula-card" href="https://txgs888.q3665.com/" target="_blank" rel="noopener"><span class="formula-cover"><img src="'+esc(image)+'" alt="'+esc(label)+'"></span><strong class="formula-name">'+esc(label)+'</strong></a>';}).join('');};
    renderDirectCards();
    fetch('https://txgs888.q3665.com/api/formula-recommendations?lotteryType='+type(),{cache:'no-store'}).then(r=>r.ok?r.json():Promise.reject()).then(payload=>{
      const rank={zodiac:0,tema:1,pingte:2,fushi:3,danshuang:4,wave:5,wuxing:6,jiaye:7,kill:8,size:9,tail:10,head:11};
      const rows=(Array.isArray(payload.recommendations)?payload.recommendations:[]).filter(row=>row.typeName!=='平特二肖').sort((a,b)=>(rank[a.board]??99)-(rank[b.board]??99)||Number(a.slot)-Number(b.slot));
      if(!rows.length)return;
      list.innerHTML=rows.map(row=>{const label=row.typeName||row.cardName||'公式推荐',image=String(row.thumbnailUrl||row.imageUrl||'').replace(/^https?:\/\/liuhe-formula-poster(?:\.workers\.dev)?/,'https://txgs888.q3665.com'),raw=String(row.url||row.href||''),href=(raw.startsWith('/posts/')?'https://txgs888.q3665.com'+raw:raw.replace(/^https?:\/\/liuhe-formula-poster(?:\.workers\.dev)?/,'https://txgs888.q3665.com'))||'https://txgs888.q3665.com/',streak=Number(row.recentStreak)||0;return '<a class="formula-card" href="'+esc(href)+'" target="_blank" rel="noopener"><span class="formula-cover"><img src="'+esc(image)+'" alt="'+esc(label)+'"></span><strong class="formula-name">'+esc(label)+'</strong><em class="formula-streak">连准 '+streak+' 期</em></a>';}).join('');
    }).catch(()=>{});
  }
...
(function(){
  const types=[1,5,8],esc=value=>String(value??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));let link='https://6hgs01z.duoldq.com/promo-login/?channelCode=883';
  let byType={};try{byType=JSON.parse(document.getElementById('initialBackendData')?.textContent||'{}').specialMaterialsByType||{};}catch{}
  const type=()=>types.includes(Number(localStorage.getItem('lotteryType')))?Number(localStorage.getItem('lotteryType')):5;
  const data=row=>{try{return JSON.parse(row.content_json||'{}')}catch{return {}}},num=value=>String(Number(value)||0).padStart(2,'0'),isHit=(group,open)=>Array.isArray(group)&&group.length===3&&group.every(value=>(open||[]).map(Number).includes(Number(value)));
  function render(rows){
    const groups={};(rows||[]).forEach(row=>(groups[row.board_key]||(groups[row.board_key]=[])).push(row));Object.values(groups).forEach(list=>list.sort((a,b)=>Number(a.id)-Number(b.id)));
    const two=document.querySelector('[data-section-key="two-code"]'),inside=document.querySelector('[data-section-key="inside-code"]'),six=document.querySelector('[data-section-key="sixgroup"]');
    const twoRows=groups.two_code||[];
    if(two&&twoRows.length){const current=twoRows.at(-1),history=twoRows.slice(0,-1);two.innerHTML='<h2 class="two-code-title">123六合网·二肖二码·123LH.COM</h2><div class="two-code-list">'+history.map(row=>{const picks=data(row).picks||[];return '<div class="two-code-row two-code-history"><span>'+esc(row.period)+'期:『二肖二码』【'+picks.map((item,index)=>'<'+(index?'i':'b')+'>'+esc(item[0])+num(item[1])+'</'+(index?'i':'b')+'>').join(' + ')+'】已公开</span></div>'}).join('')+'<div class="two-code-row pending two-code-current"><span class="two-code-current-line">'+esc(current.period)+'期:『二肖二码』【<a id="twoCodeCountdown" href="'+link+'" target="_blank" rel="noopener noreferrer">立即领取 <b>--:--:--</b></a>】开:發00准</span></div><div class="two-code-footer"><div class="two-code-promo-card"><p>❓想赢钱，先把资料领到手<br><strong>精准资料，限时免费领取</strong></p><div class="two-code-promo-points"><span>🎁 免费领资料 + 红包</span><span>✔ 中奖即提，不中再补一次</span><span>别再犹豫，马上领取！</span></div><div class="two-code-promo-actions"><a class="two-code-giveup" href="'+link+'" target="_blank" rel="noopener noreferrer">放弃发财</a><a class="two-code-claim" href="'+link+'" target="_blank" rel="noopener noreferrer">点击暴富</a></div></div></div></div>'}
    const insideRows=groups.inside_code||[];
    if(inside&&insideRows.length){const current=insideRows.at(-1),history=insideRows.slice(0,-1);const card=row=>{const values=data(row).numbers||[];return '<article class="inside-code-card"><div class="inside-code-head"><span class="inside-code-mark">六合彩<br>MARK SIX</span><strong>第'+esc(row.period)+'期</strong><span class="inside-code-tag">资料<small>123LH</small></span></div><div class="inside-code-numbers">'+values.slice(0,2).map((value,index)=>'<span class="inside-code-number'+(index===0?' hot':'')+'">'+num(value)+'</span>').join('')+'</div></article>'};inside.innerHTML='<h2 class="inside-code-title">123六合网·<b>内幕二码</b>·123LH.COM</h2><div class="inside-code-grid">'+history.map(card).join('')+'<article class="inside-code-card pending"><div class="inside-code-head"><span class="inside-code-mark">六合彩<br>MARK SIX</span><strong>第'+esc(current.period)+'期</strong><span class="inside-code-tag">资料<small>123LH</small></span></div><div class="inside-code-reveal"><span>揭</span><span>秘</span></div></article></div><a class="inside-code-ad" href="'+link+'" target="_blank" rel="noopener noreferrer"><strong>点击提前领取资料</strong></a>'}
    const sixRows=groups.sixgroup||[];
    if(six&&sixRows.length){const current=sixRows.at(-1),history=sixRows.slice(0,-1),card=row=>{const value=data(row),open=value.open||[],groups=value.groups||[];return '<article class="sixgroup-card"><h3>'+esc(row.period)+'期六组三中三</h3><div class="sixgroup-lines">'+groups.map(group=>'<span class="'+(isHit(group,open)?'hot':'')+'">'+group.map(num).join('·')+'</span>').join('')+'</div>'+(open.length?'<p class="sixgroup-open">开:<b>'+open.map(num).join('·')+'</b> 准</p>':'')+'</article>'};six.innerHTML='<h2 class="sixgroup-title">123六合网·六组三中三·123LH.COM</h2><div class="sixgroup-list">'+history.map(card).join('')+'<article class="sixgroup-card current"><h3>'+esc(current.period)+'期六组三中三</h3><p class="sixgroup-free">【<b>点击免费提前查看三中三</b>】</p><p class="sixgroup-app">点击提前领取独家资料</p><a class="sixgroup-cta" href="'+link+'" target="_blank" rel="noopener noreferrer">点击提前领取资料</a></article></div>'}
  }
  async function apply(selected){const cached=byType[selected]||byType[String(selected)];if(Array.isArray(cached)&&cached.length){render(cached);return}try{const response=await fetch('/api/public/special-materials?lotteryType='+selected,{cache:'no-store'}),payload=await response.json();if(response.ok&&payload.success){byType[selected]=payload.data||[];render(byType[selected])}}catch{}}
  async function loadRegistrationLink(){try{const response=await fetch('/api/public/site-settings',{cache:'no-store'}),payload=await response.json(),value=String(payload?.data?.registration_url||'').trim();if(/^https?:\/\//i.test(value))link=value;}catch{}}
  loadRegistrationLink().finally(()=>apply(type()));document.querySelectorAll('#lotteryMenu button,.lottery-tab').forEach(button=>button.addEventListener('click',()=>setTimeout(()=>apply(type()),0)));
})();
