(function(){
  ...
  function renderMemberPosts(records){const grid=document.querySelector('.master-grid');if(!grid)return;grid.querySelectorAll('.member-post-row').forEach(node=>node.remove());const html=(records||[]).map(item=>{const title=(masterCurrentPeriod||'最新')+'期: '+item.author+' → '+item.post_type+' ←独家资料';return '<a class="member-post-row" href="/member-post.html?id='+Number(item.id)+'">'+esc(title)+'</a>'}).join('');grid.insertAdjacentHTML('afterbegin',html)}
