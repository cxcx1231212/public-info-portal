(function(){
  ...
  let initialByType={},initialMasterPostsByType={},initialMemberPostsByType={};
  ...
  function renderInitialMasterBoard(){const grid=document.querySelector('#masters .master-grid');if(!grid)return;const type=currentLotteryType(),posts=initialMasterPostsByType[type]||initialMasterPostsByType[String(type)]||[],period=Math.max(0,...posts.map(item=>Number(item.period)||0)),current=posts.filter(item=>Number(item.period)===period).sort((a,b)=>Number(a.rank_no)-Number(b.rank_no)||Number(a.master_id)-Number(b.master_id));if(!current.length){grid.innerHTML='';grid.classList.remove('is-pending');return;}grid.innerHTML=current.map(row=>{const rank=Number(row.rank_no)||1;return '<a class="master-card master-line-card" data-master-id="'+esc(row.master_id)+'" href="master-detail.html?id='+encodeURIComponent(row.master_id)+'&lotteryType='+type+'"><span class="master-line"><span class="master-line-period">'+esc(row.period)+'期:</span><span class="master-line-author">'+esc(cleanMasterName(row.name)||'高手')+'</span><span class="master-line-category">→【'+esc(row.specialty||'独家资料')+'】</span><span class="master-line-slogan">←'+esc(masterSlogans[(rank-1)%masterSlogans.length])+'</span></span></a>';}).join('');const members=[...(initialMemberPostsByType[type]||initialMemberPostsByType[String(type)]||[])];members.sort((a,b)=>Number(a.id)-Number(b.id)).forEach(item=>{const html='<a class="member-post-row" href="/member-post.html?id='+Number(item.id)+'&lotteryType='+type+'">'+esc(period)+'期: '+esc(item.author||'会员资料')+' → '+esc(item.post_type||'会员贴')+' ←独家资料</a>',targets=[...grid.querySelectorAll('.master-card')],target=targets[Number(item.id)%Math.max(1,targets.length+1)];if(target)target.insertAdjacentHTML('beforebegin',html);else grid.insertAdjacentHTML('beforeend',html);});grid.classList.remove('is-pending');}
  function hydrate(records,masters){state.content.clear();masterCurrentPeriod=Math.max(0,...(records||[]).filter(record=>record.section_key!=='threeperiod').map(record=>Number(record.period)||0));(records||[]).forEach(record=>{if(!state.content.has(record.section_key))state.content.set(record.section_key,[]);state.content.get(record.section_key).push(record);});state.ready=true;document.querySelectorAll('[data-section-key]').forEach(section=>{const key=section.dataset.sectionKey;if(state.content.has(key))renderSection(section,latestButton(section,key));});renderInitialMasterBoard();document.body.classList.remove('backend-loading');}
